...
for epoch in range(NUM_EPOCHS):
    ...
    z_latent = z_latent.view(-1, 1, 3).detach().cpu().numpy()
    save_image(z_latent, os.path.join(OUTPUT_DIR, f'latent_epoch_{epoch+1}.png'), 'Latent', reshape=(-1, 1, 3))
    np.save(os.path.join(OUTPUT_DIR, f'latent_epoch_{epoch+1}.npy') , z_latent.squeeze())

# save the model
torch.save(model.state_dict(), f'{OUTPUT_DIR}/lae.pth')
